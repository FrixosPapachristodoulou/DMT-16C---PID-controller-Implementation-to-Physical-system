...
const float minFlowRate = targetFlowRate * 0.95; // Lower flow rate boundary, adjustable
const float maxFlowRate = targetFlowRate * 1.05; // Upper flow rate boundary, adjustable

// Control parameters
const float Kp = 0.054; // Increased proportional gain, adjustable
...
void loop() {
  ...
  if (vfdRunning) {
    ...
    // Start control after 180 seconds
    if (currentMillis - startTime >= 180000) {
      controlStarted = true;
    }
